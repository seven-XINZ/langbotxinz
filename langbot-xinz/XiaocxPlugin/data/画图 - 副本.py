...
def generate_image(api_key, prompt, size, steps, prompt_upsampling, seed, guidance, safety_tolerance, interval):
    # 定义生成图像的函数
    url = "https://api.ephone.ai/v1/images/generations"  # 设置 API 端点 URL
    headers = {
    ...
    body = {
        "model": "dall-e-3",  # 设置使用的模型名称
         "prompt": f"anime style, Japanese anime，{prompt}",  # 在提示词前添加“日本动漫风格”
        "size": size,  # 图像尺寸
        "steps": steps,  # 生成步骤数
...
if len(sys.argv) < 2:  # 检查是否提供了提示词
    print("请提供提示词，例如: python 画图.py 猫")  # 提示用户输入格式
    sys.exit(1)  # 退出程序

prompt = sys.argv[1]  # 获取命令行参数中的提示词

# 示例调用
api_key = "sk-oRr4Df4l4uAKOa2AgsDyuxaXyHHXZ6wN5NN4zapwuwc2Zh73"  # 你的 API 密钥
generate_image(
    api_key=api_key,  # 传入 API 密钥
    prompt=prompt,  # 使用命令行传入的提示词
    size="1024x1024",  # 图像尺寸设置为 768x1024
    steps=100,  # 设置生成步骤数为 50
    prompt_upsampling=True,  # 设置提示词上采样为 True
    seed=78,  # 设置随机种子为 42
    guidance=8,  # 指导参数设置为 7
    safety_tolerance=10,  # 安全容忍度设置为 5
